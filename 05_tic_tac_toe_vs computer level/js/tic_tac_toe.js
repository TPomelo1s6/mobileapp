...
function pandaTurn() {
    ...
    while (pandaTurnEnd === "0") {

        if (level === "1" || level === "2" || level === "3") {
            // パンダのリーチ行検索
            pandaTurnEnd = isReach("panda");
            if (pandaTurnEnd === "1") {
                gameOverFlag = "1";
                break;
            }
    ...
    lineArray.some(function (line) {
        ...
        line.forEach(function (square) {
            if (square.classList.contains("js-cat-checked")) {
                // 猫の数をカウント
                catCheckCnt++;
            } else if (square.classList.contains("js-panda-checked")) {
                // パンダの数をカウント
                pandaCheckCnt++;
            }
        });

        // パンダのリーチ行検索ときに、パンダのリーチ行あり
        if (status === "panda" && pandaCheckCnt === 2 && catCheckCnt === 0) {
            pandaTurnEnd = "1";
        }

        // 猫のリーチ行検索ときに、猫のリーチ行あり
        if (status === "cat" && pandaCheckCnt === 0 && catCheckCnt === 2) {
            pandaTurnEnd = "1";
        }

        // パンダが猫のリーチ行アリの場合、空いているマス目を選択する
        if (pandaTurnEnd === "1") {
...
function levelSetting(index) {
    ...
    // sessionStorage　key = "tic_tac_toe_access"がある場合
    if (sessionStorage.getItem("tic_tac_toe_access")) {
        ...
        // sessionStorage　key = "tic_tac_toe_access"がない場合、初期化
        sessionStorage.setItem("tic_tac_toe_access", "1");
        level_1.classList.add("level-selected");
        level_2.classList.add("level-non-selected");
        level_3.classList.add("level-non-selected");
